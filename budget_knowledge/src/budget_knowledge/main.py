...
from .crews.budget_or_internet_check_crew.budget_or_internet_crew import (
    UnionBudgetCheckCrew,
)
...
class UnionBudgetFlow(Flow[UnionBudgetState]):
    ...
    def trigger_crews(self):
        ...
        self.state.query_type = (
            UnionBudgetCheckCrew().crew().kickoff(inputs={"query": self.state.query})
        )
        print("Query Typeee ---->>>  ", self.state.query_type)
        if self.state.query_type["union_budget_flag"] == "union":
            print(
                "You are asking UNION BUDGET related query, Let me answer you clearly !!\n\n\n"
            )
            response = (
                UnionBudgetCrew().crew().kickoff(inputs={"query": self.state.query})
            )
        elif self.state.query_type["union_budget_flag"] == "NA":
            print(
                "This is the query beyond my knowledge so, Let use the internet and answer the query\n\n\n"
            )
            response = InternetCrew().crew().kickoff(inputs={"query": self.state.query})
        else:
            print(
                "The query type is not recognized. Please ensure your input is valid."
            )
